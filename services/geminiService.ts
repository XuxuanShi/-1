...
export const generateBatchContent = async (topic: string, count: number = 3) => {
  try {
    const apiKey = process.env.API_KEY;
    if (!apiKey) {
      throw new Error("API Key not found");
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      contents: `Generate ${count} distinct and useful Cantonese words or phrases related to the topic: "${topic}". Ensure they are suitable for a learner.`,
      config: {
        systemInstruction: systemInstruction,
        responseMimeType: "application/json",
        responseSchema: {
